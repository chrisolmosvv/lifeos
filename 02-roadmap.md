...
three-mode edit/delete scope reusing the calendar's SeriesScopePrompt verbatim). Ledger capped at
today. Repeat-pattern editing not supported (same constraint as calendar). Event/task regression
clean across both 6c commits.
**NEXT: Piece 7 — Budgets (per-category monthly limits, "everything else" line, brick bar).**
...
longer dormant); (5) editor remap (the corruption fix — references follow steps through reorder/
add/delete) + confirm surface (show the plan, reassign ingredient links, edit dependencies).
No schema change. recipe-import Edge Function redeployed.
**DEBT (Step 7 pile):**
- ~39% of ingredients are GENERAL (no step match) — now OWNER-FIXABLE via the confirm surface.
  Re-importing a recipe uses the tightened heuristic and is the alternative workaround.
