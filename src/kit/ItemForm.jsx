...
import SeriesScopePrompt from './SeriesScopePrompt'
import { useRepeat } from './RepeatField'
import { buildRecipe, buildOneOffFields } from '../recur/recipe'
...
export default function ItemForm({ kind, item, create, cats, inboxColor, busy, onSave, onSaveSeries, onSaveSeriesEdit, onDelete, onClose, subtasks, onSubtask, parentLabel }) {
  ...
  // Editing a LIVE occurrence of a series (series_id set, not yet detached) → save
  // asks the scope. A one-off (series_id null), an already-detached occurrence, or a
  ...
  async function save() {
    ...
    const built = buildOneOffFields(k, { ttl, notes, categoryId, status, priority, due, schStart, schEnd, bucket, allDay, startDate, endDate, startAt, endAt, location })
    if (built.error) return setErr(built.error)
    // Editing an occurrence of a series → ask the scope (this one / following / all);
    // a one-off (or a create) saves directly.
    if (isSeriesOcc) { setScopeFields(built.fields); return }
    const msg = await onSave(built.fields, k)
    if (msg) setErr(msg)
  }
