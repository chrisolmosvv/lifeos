...
export async function createBirthdaySeries(personName, dateValue) {
...
export async function syncBirthdayCalendar(dateRowId, personName, dateValue, showOnCalendar, existingRecurrenceId) {
  if (showOnCalendar) {
    // Retire old if exists, then create fresh
    if (existingRecurrenceId) await retireBirthdaySeries(existingRecurrenceId)
    const newId = await createBirthdaySeries(personName, dateValue)
