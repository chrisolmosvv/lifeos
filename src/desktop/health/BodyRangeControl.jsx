...
// LifeOS — Body V3 (Piece 9): the Body time control. Today / 3 Months / 6 Months / 1 Year,
// with prev/next paging (active only on the windowed views), a date-range label, and a
// "back to today" shortcut once paged away. Body-LOCAL — reuses the shared RangeSwitcher for
// the tabs but owns its own paging chrome; it does NOT touch Sleep's or Gym's switchers.
...
export default function BodyRangeControl({
  ...
  return (
    <div className="brc">
      ...
      <RangeSwitcher ranges={BODY_RANGES} value={win} ariaLabel="Body time range" onChange={onWin} />
      {paging && (
